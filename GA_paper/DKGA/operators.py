from functions import detect_obstacle_plus, detect_obstacle
import random
...
def refinement(path, MAP):
    ...
    while index < len(path) - 2:
        ...
        if (x1 == x2 and y2 == y3) or (y1 == y2 and x2 == x3):  # if there is a right angle
            ...
            if detect_obstacle(path[index], path[index + 2], MAP):  # two nodes can be connected directly
                path.remove(path[index + 1])
                index -= 1
            else:
                ...
                while index2 < len(nodes_middle1) and index2 < len(nodes_middle2):
                    if detect_obstacle(nodes_middle1[index2], nodes_middle2[index2], MAP):
