from functions import detect_obstacle_plus, no_obstacle
import random
from functions import generate_all_middle_nodes, generate_middle_nodes
import config
    ...
    if random.random() < 0.5:
        ...
        while index < len(path) - 2:
            if random.random() < config.P_D:
                obstacle_flag, nodes_middle = detect_obstacle_plus(path[index], path[index + 2], MAP)
                if obstacle_flag:
...
def refinement(path, MAP):
    ...
    while index < len(path) - 2:
        ...
        if (x1 == x2 and y2 == y3) or (y1 == y2 and x2 == x3):  # if there is a right angle
            ...
            if no_obstacle(path[index], path[index + 2], MAP):  # two nodes can be connected directly
                path.remove(path[index + 1])
                index -= 1
            else:
                if x1 == x2 and y2 == y3:  # possible shape of right angle
                    ...
                    if y1 < y2:
                        nodes_middle1 = [(x1, y1 + i) for i in range(y2 - y1)]
                    else:
                        nodes_middle1 = [(x1, y1 - i) for i in range(y1 - y2)]
                    if x2 > x3:
                        nodes_middle2 = [(x3 + i, y2) for i in range(x2 - x3)]
                    else:
                        nodes_middle2 = [(x3 - i, y2) for i in range(x3 - x2)]
                else:  # the other possible shape of right angle (y1 == y2 and x2 == x3)
                    if x1 < x2:
                        nodes_middle1 = [(x1 + i, y1) for i in range(x2 - x1)]
                    else:
                        nodes_middle1 = [(x1 - i, y1) for i in range(x1 - x2)]
                    if y2 > y3:
                        nodes_middle2 = [(x2, y3 + i) for i in range(y2 - y3)]
                    else:
                        nodes_middle2 = [(x2, y3 - i) for i in range(y3 - y2)]

                # find a shorter path
                index2 = 1
                flag = 0 # no refinement
                while index2 < len(nodes_middle1) and index2 < len(nodes_middle2):
                    if no_obstacle(nodes_middle1[index2], nodes_middle2[index2], MAP):
