...
from operators_single_target import detect_cycle,circuit_removal, insertion_deletion, refinement
from functions import calculate_coordinate, calculate_number, plot_paths, plot_path_single
import config
...
def selection(pop, scores):
    ...
    new_pop = []
    test = randint(2,60)
    index = [random() for i in range(test)]
    index.sort()
    pointer = 0
    pos_wheel = 0
    while (pointer < test):
...
# genetic algorithm
def genetic_algorithm(MAP, pop):
    # keep track of best solution
    best, shortest_length = 0, float("Inf")
    # best_fitness = shortest_length + evaluation_smooth(pop[0], MAP_SIZE)
    best_fitness = shortest_length
    best_fitness_gen_before = None # elitist strategy
    # iterations
    for gen in range(config.ITERATION):
        # evaluate all candidates in the population
        route_length = [evaluation(path, MAP) for path in pop]
        # smoothness = np.array([evaluation_smooth(c, MAP_SIZE) for c in pop])  # consider the smoothness of the path
        # fitness = root_length + smoothness
        fitness = route_length
        ...
        if best_fitness_current_gen < best_fitness:
            best, best_fitness, shortest_length = pop[index_best], fitness[index_best], route_length[index_best]
        ...
        #     print(j)
        # print()
        # for index in range(len(new_pop)):
        #     detect_cycle(new_pop[index], gen)
    return [best, best_fitness, pop]
